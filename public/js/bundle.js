...
var displayMap = function displayMap(locations) {
  mapboxgl.accessToken = 'pk.eyJ1IjoiaW1yaXNoYXYiLCJhIjoiY2s5aXJrczkzMDVpbTNkcXM0dzEycHFudCJ9.T8vbFOV2XWM0JvZRz89PXQ';
  ...
  locations.forEach(function (log) {
    //adding marker,
    ...
    new mapboxgl.Marker({
      element: el,
      anchor: 'bottom'
    }).setLngLat(log.coordinates).addTo(map); //Add Popup

    new mapboxgl.Popup({
      offset: 30
    }).setLngLat(log.coordinates).setHTML("<p>Day ".concat(log.day, ": ").concat(log.description, "</p>")).addTo(map); //Extends map bounds
    ...
    return regeneratorRuntime.wrap(function _callee$(_context) {
      while (1) {
        switch (_context.prev = _context.next) {
          case 0:
            _context.prev = 0;
            url = type === 'password' ? 'http://localhost:3002/api/v1/users/updatePassword' : 'http://localhost:3002/api/v1/users/updateMe';
            _context.next = 4;
            return (0, _axios.default)({
              method: 'PATCH',
              url: url,
          ...
          case 4:
            ...
            if (res.data.status === 'success') {
              (0, _alert.showAlert)('success', "".concat(type.toUpperCase(), " Data Update Successfully"));
            }
...
console.log('connected...'); //Dom Elemnets
...
if ((!parent || !parent.isParcelRequire) && typeof WebSocket !== 'undefined') {
  ...
  var ws = new WebSocket(protocol + '://' + hostname + ':' + "38335" + '/');
